...
For ambr, I utilized Sartorius's own power number for the standard baffled mammalian vessel with dual 26 mm impellers, $N_p=1.34$, reported in [Rotondi et al. (2021)](https://doi.org/10.1007/s10529-021-03076-3) with Sartorius Stedim co-authors and stated again in Sartorius patent [US 12540362 B2](https://patents.google.com/patent/US12540362B2/en). For the mixing number, I took the plateau that [Miranda et al. (2026)](https://doi.org/10.1016/j.cherd.2026.01.030) measured for the same configuration, the commercial dual impeller in the baffled vessel without probes, in down-pumping mode: $N t_m=37.75$ ([UCL manuscript](https://discovery.ucl.ac.uk/id/eprint/10220724/1/ambr250.pdf)). Notably, adding a pH probe and sparger line raised their measurements to $N_p=2.12$ and $N t_m=57.64$, so the clean-vessel pair is the one consistent with the manufacturer's power number. The time at 300 rpm is therefore $37.75/(300/60)=7.55$ s. With tank diameter 61 mm, impeller diameter 26 mm, and liquid-height ratio 1.41 from their section 2.1/Figure 1, the correlation introduced below gives a baseline mixing number of $N t_{95,GN}=30.83$, and dividing 37.75 by this baseline gives $f_c=1.22$.

For the other datasets, I calculated mixing numbers as $(\mathrm{rpm}/60)t_{95}$; speeds inferred from tip velocity depend on the assumed impeller diameter. For the XDR vessels, I applied Cytiva's own power numbers from its XDR characterization, $N_p$ = 0.90, 0.97, 0.89, and 0.51 for XDR-200, XDR-500, XDR-1000, and XDR-2000, which Cytiva publishes through its [Bioreactor Scaler](https://bioreactorscaler.cytivalifesciences.com/) and confirmed in customer correspondence in July 2024; the XDR-500 and XDR-1000 application notes cited above report the same plateaus, approximately 0.98 and 0.9. Tank and impeller dimensions come from [EP2663629B1, Table 1](https://patents.google.com/patent/EP2663629B1/en). For the Biostat STR and UniVessel SU, I applied Sartorius's own value for the pair of 3-blade segment impellers, $N_p=1.3$, reported by [De Wilde et al. (2014)](https://www.bioprocessintl.com/single-use/superior-scalability-of-single-use-bioreactors) of Sartorius Stedim Biotech. The scaled STR geometry and the effective cylindrical UniVessel geometry are approximations.
